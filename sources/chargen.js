...
try {
 ...
 for (const categoryType of selectedCategories) {
   try {
     ...
     if (!categoryFolder) {
       failureDetails.push(`Failed to create folder for category: ${categoryType}`);
       continue;
     }
...
// Populate category checkboxes after everything is loaded
setTimeout(populateCategoryCheckboxes, 1000);
});