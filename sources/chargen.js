...
// FIXED CATEGORY EXPORT FUNCTION
...
try {
  ...
  if (!itemCtx) {
    throw new Error("Failed to get canvas context for item rendering");
  }
  ...
  categoryItems.each(function() {
    ...
    // Skip items without required attributes
    ...
    // Get file path - FIXED: proper data attribute access
    const fileName = $item.data(`layer_1_${bodyType}`) || "";
  ...
  // Validate we have subcategories to export
  if (Object.keys(subcategories).length === 0) {
    alert("No valid items found to export for this category.");
    return;
  }
  ...
  for (const [subcategoryName, items] of Object.entries(subcategories)) {
    ...
    if (!subcategoryFolder) {
      ...
      failedItems.push(...items.map(item => `${subcategoryName}_${item.variant}`));
      continue;
    }
    ...
    for (const itemToDraw of items) {
      try {
        // Clear canvas and draw the item
        itemCtx.clearRect(0, 0, itemCanvas.width, itemCanvas.height);
        drawItemSheet(itemCanvas, itemToDraw, addedCustomAnimations || []);

        // Generate filename and save to zip
  ...
  // Show result message
  if (failedItems.length > 0) {
    alert(`Export completed with issues:\nSuccessfully exported: ${exportedItems.length} spritesheets\nFailed: ${failedItems.join(', ')}`);
  } else {
